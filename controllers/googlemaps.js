...
module.exports = function (zipCode, type, miles, cb) {
    ...
    // var origins = ['Brooklyn Park, MN'];
    // var destinations = ['Minneapolis, NY'];
            ...
            googleMapsClient.placesNearby({
                ...
                if (err) { throw err }
                    ...
                    for (var i = 0; i < response.json.results.length; i++) {
                        //response.json.results.length
                        // console.log(response.json.results[0]);
                        //console.log(response.json.results[i].id);

                      var place = {};
                        place.placeID = response.json.results[i].place_id;
                        place.placeNames = response.json.results[i].name;
                        // console.log(response.json.results[i].opening_hours);
                        ...
                        //console.log(response.json.results[i].types[2]);
                        place.placeAdress = response.json.results[i].vicinity;
                        //console.log(response.json.results[0].photos[0].html_attributions);
