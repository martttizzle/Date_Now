...
let googleMapsClient = googleMethodes.googleMapsClient;
    ...
    for (i in searchInput) {
        ...
        googleMapsClient.place({
            ...
            if (!err) {
                ...
                place.open = googleMethodes.openNow(response.json.result.opening_hours);
                place.googleRating = response.json.result.rating;
                place.pricing = googleMethodes.pricing(response.json.result);
                ...
                if (results.length === searchInput.length) {
                    // Get Distance range
                    googleMethodes.addRange(results, function (ranges) {
                        for (i in results) {
